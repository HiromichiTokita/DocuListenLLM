...
- ⏳ **Task 3**（VoiceDesignBackend ref生成/クローン・import確認）／**Task 5**（main.py 設定＋producer連携・py_compile）は
  GPU不要で**コードは書ける**（実行/E2EがGPU）。**Task 6**（🎲/声固定UI・手動）／**Task 7**（バンドル更新＋E2E・GPU）。
