...
- ⏳ **2b E2E**は下記 C に統合（refはtorchcodecブロッカーで未達だった→C-Task1で解消）。

#### サブPJ-2C（参照音声をref_latent化＋カテゴリ毎エンジン振り分け）— 2026-06-27
**経緯**: 2bのE2Eで「声がばらばら」のまま。ログで原因判明＝**ref_wav 読込が torchaudio→torchcodec を要求し、
ffmpeg無し環境で失敗→毎回 caption-only にフォールバック**していた（音は出るが声固定されず）。
torchcodecはWindowsでffmpeg依存があり面倒なため**ref_latent方式（torchcodec/ffmpeg不要）**へ転換。併せて**カテゴリ毎にエンジン選択**（VOICEVOX/Irodori）も追加。
- 設計: `docs/superpowers/specs/...`（2bを継承）／計画: `docs/superpowers/plans/2026-06-27-irodori-mixed-engine-and-ref-latent.md`
- ✅ **C-Task1**（IrodoriVDServer, `3c39e5e`）: `_generate_ref` を `codec.encode_waveform`→latent→`torch.save(.pt)`、
  `_synthesize_ref` を `ref_latent=path` に（torchcodec不要）。バンドルも更新。20 tests green。
  ※ `encode_waveform` のshape前提（(C,T)→(1,T,D)、保存(T,D)）は**GPU実走で未検証**＝C-Task5の要確認ポイント。
- ✅ **C-Task2**（DocuListenLLM, `cf75a4e`）: `engine_for(category, global, category_engines)` 純粋関数（21 tests green）。
